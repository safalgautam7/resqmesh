            ...
            override fun onConnectionStateChange(
                ...
                when (newState) {
                    BluetoothProfile.STATE_CONNECTED -> {
                        // Bondless mesh: accept the relay connection immediately
                        // without forcing device.createBond(). The relay write
                        // characteristic is non-secure (no link-layer encryption
                        // required), so data passes over an unencrypted GATT link
                        // and the OS never pops a pairing dialog. This is essential
                        // for the "unconscious sender" use case where nobody can
                        // tap "Pair" on either device.
                        handler?.post {
